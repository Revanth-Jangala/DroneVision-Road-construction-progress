...
def validate_metadata(coords1, coords2, alt1, alt2, ts1, ts2, img1_brightness, img2_brightness):
    try:
        ...
        # Altitude difference
        if abs(alt1 - alt2) > 10:
            return False, "Altitude difference must be less than 10 meters."
        ...
        # Brightness check
        if img1_brightness < 80 or img2_brightness < 80:
            return False, "Both images must have brightness greater than 80. Please upload clearer images."
...
def process_and_compare(image1, image2, coords1, coords2, alt1, alt2, ts1, ts2):
    try:
        ...
        img1_brightness = compute_brightness(img1)
        img2_brightness = compute_brightness(img2)

        # Metadata validation
        valid, message = validate_metadata(coords1, coords2, alt1, alt2, ts1, ts2, img1_brightness, img2_brightness)
        if not valid:
